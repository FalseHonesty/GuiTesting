...
public class NahrFont {
    ...
    private void createFont(Object font, float size) {
        try {
            if (font instanceof Font) {
                ...
                Font myFont = Font.createFont(0, (InputStream) font);
                this.theFont = myFont.deriveFont(size);
    ...
    private void drawer(String text, float x, float y, int color) {
        ...
        for (int i = 0; i < text.length(); i++)
            if (text.charAt(i) == '\247' && i + 1 < text.length()) {
                ...
                final int colorCode = Minecraft.getMinecraft().fontRendererObj.getColorCode(oneMore);
    ...
    private void drawTexturedModalRect(float x, float y, float u, float v, float width, float height) {
        ...
        worldrenderer.pos((double)(x + 0), (double)(y + height), 0).tex((double)((u + 0) * f), (double)((v + height) * f1)).endVertex();
        worldrenderer.pos((double)(x + width), (double)(y + height), 0).tex((double)((u + width) * f), (double)((v + height) * f1)).endVertex();
        worldrenderer.pos((double)(x + width), (double)(y + 0), 0).tex((double)((u + width) * f), (double)((v + 0) * f1)).endVertex();
        worldrenderer.pos((double)(x + 0), (double)(y + 0), 0).tex((double)((u + 0) * f), (double)((v + 0) * f1)).endVertex();
        tessellator.draw();
    }
